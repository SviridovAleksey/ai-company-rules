...
---
name: fix-issue
description: Fix a GitHub/Jira issue following corporate standards
required-for:
  levels: [all]
  types: [all]
---
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "PROJECT_LINT_CMD=${CLAUDE_LINT_CMD:-npm run lint} && $PROJECT_LINT_CMD -- $CLAUDE_FILE_PATH 2>/dev/null || true"
          }
        ]
      }
    ]
...
> Переменная окружения `CLAUDE_LINT_CMD` позволяет переопределить команду линтера под стек проекта:
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "echo $CLAUDE_FILE_PATH | grep -q '^docs/' && echo 'BLOCK: docs/ files require explicit human approval. Ask the user first.' && exit 1 || exit 0"
          }
        ]
      }
    ]
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "echo $CLAUDE_FILE_PATH | grep -qv '\\.test\\.' && echo $CLAUDE_FILE_PATH | grep -qv '\\.spec\\.' && PROJECT_TEST_CMD=${CLAUDE_TEST_CMD:-npm test} && $PROJECT_TEST_CMD 2>/dev/null || true"
          }
        ]
      }
    ]
...
> Переменная окружения `CLAUDE_TEST_CMD` позволяет переопределить команду тестов:
...
Toolkit подключается как **git submodule** в директорию `.claude/toolkit`:

```bash
# 1. Добавить submodule (при инициализации проекта)
git submodule add https://git.company.internal/ai/company-ai-toolkit .claude/toolkit
...
# 3. Запустить импорт — скопировать нужные файлы в .claude/
.claude/toolkit/import.sh --level L2 --type backend
git add .claude/
...
> Файлы в `.claude/skills/`, `.claude/agents/`, `.claude/rules/` — **копии**, а не симлинки. Это обеспечивает работу на всех ОС включая Windows.
...
.claude/toolkit/import.sh --level L2 --type backend --update
...
update-ai-toolkit:
  ...
  script:
    - cd .claude/toolkit && git fetch --tags
    ...
    - |
      if [ "$CURRENT" != "$LATEST" ]; then
        git checkout $LATEST
        cd ../..
        .claude/toolkit/import.sh --level $PROJECT_LEVEL --type $PROJECT_TYPE --update
  ...
  update-toolkit:
    ...
    steps:
      ...
      - name: Apply update if available
        ...
        run: |
          cd .claude/toolkit && git checkout ${{ env.LATEST }} && cd ../..
          .claude/toolkit/import.sh --level ${{ vars.PROJECT_LEVEL }} --type ${{ vars.PROJECT_TYPE }} --update
      ...
      - name: Create Pull Request
        ...
        with:
          ...
          body: |
            ...
            Review the diff in `.claude/` before merging.
```
...
3. Копирует обязательные файлы в `.claude/` проекта
4. Генерирует `.mcp.json` по уровню
5. Смерджирует hooks в `.claude/settings.json`
6. Выводит список импортированных и пропущенных файлов

### Ручной

1. Скопировать файлы из `core/` в `.claude/` проекта
2. Скопировать файлы специфичные для типа проекта
3. Для L3 — добавить файлы из `l3/`
4. Смержить hooks в `.claude/settings.json`
...
> Отдел по внедрению ИИ публикует release notes в Confluence при каждом релизе toolkit'а. MAJOR-релизы сопровождаются migration guide.
